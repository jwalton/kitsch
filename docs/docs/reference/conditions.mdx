...
- `ifOS: ["darwin", "linux"] - The condition is met only if the OS is one of the OSs listed.
- `ifNotOS: ["windows"]` - The conditions is met only be shown if the OS is not one of the listed OSs.

Note that "ifFile", "ifAncestorFile", and "ifExtension" are "or"ed together - if any one of these conditions is met, the "conditions" block is considered met.. The "OS" conditions are "and" conditions - if they are not met, the conditions will not be fulfilled, even if files or extensions match.
